...
abstract contract BaseStrategy is AccessControlMixin {
    ...
    function getWantsInfo() external view virtual returns (address[] memory _assets,uint[] memory _ratios);
    ...
    function claimRewards() internal virtual returns (address[] memory _rewardTokens,uint256[] memory _claimAmounts);

    /// @notice Report asset change results and claim information 
    function report(address[] memory _rewardTokens,uint256[] memory _claimAmounts) private {
        ...
        emit Report(prevTotalAsset,currTotalAsset,_rewardTokens,_claimAmounts);
    }
    ...
    function repay(uint256 _repayShares,uint256 _totalShares) external returns (address[] memory _assets, uint256[] memory _amounts){
    ...
    /// @param _totalShares Denominator
    function withdrawFrom3rdPool(uint256 _withdrawShares,uint256 _totalShares) internal virtual returns (address[] memory _assets, uint256[] memory _amounts);
    ...
    function sweep(address _token) isKeeper external {
    ...
    /// @notice Query the vaule of Token.
    function queryTokenVaule(address _token,uint256 _amount) internal view returns (uint256 vauleInUSD){
    ...
    function transferTokensToTarget(address _target,address[] memory _assets, uint256[] memory _amounts) internal {
        for (uint8 i = 0; i < _assets.length; i++) {
