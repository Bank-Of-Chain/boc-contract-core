...
contract CustomSEthPriceFeed is ICustomPriceFeed {
    ...
    function _sEthPriceInEth() private view returns (uint256) {
        //curve price
        uint256 _price1 = ICustomSEthCurveFi(SETH_ETH_CURVE_POOL).get_dy(1, 0, 1e18);
        ...
        uint256 _price2 = uint256(_sUSDOfUSD) * 1e10;
        uint256 _delta;
        uint256 _priceHigh;
        if (_price1 > _price2) {
            _delta = _price1 - _price2;
            _priceHigh = _price1;
        } else {
            _delta = _price2 - _price1;
            _priceHigh = _price2;
        }
        require(_priceHigh * 8 > 1000 * _delta, "delta too much");
        // 0.9 price1 + 0.1 price2
        return (_price1 * 9) / 10 + _price2 / 10;
    }
}
