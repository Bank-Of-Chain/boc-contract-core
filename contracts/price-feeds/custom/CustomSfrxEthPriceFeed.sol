...
interface ICustomSfrxEthCurveFi {
    function price_oracle() external view returns (uint256);
}

interface ICustomSfrxEthSfrxEth {
    function pricePerShare() external view returns (uint256);
}

contract CustomSfrxEthPriceFeed is ICustomPriceFeed {
    ...
    function calcValueInUsd(uint256 _amount) external view override returns (uint256 _valueInUsd) {
        ...
        uint256 _priceInETH = _sfrxEthPriceInEth();
    ...
    function _frxEthPriceInEth() private view returns (uint256) {
        uint256 _priceInCurve = ICustomSfrxEthCurveFi(FRXETH_ETH_CURVE_POOL).price_oracle();
    ...
    function _sfrxEthPriceInEth() private view returns (uint256) {
        return (ICustomSfrxEthSfrxEth(SFRX_ETH).pricePerShare() * _frxEthPriceInEth()) / 1e18;
    }
}
