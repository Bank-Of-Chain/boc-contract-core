...
contract EthParaSwapV5Adapter is ParaSwapV5ActionsMixin, IExchangeAdapter, ExchangeHelpers {
    ...
    function swap(uint8 _method, bytes calldata _encodedCallArgs, SwapDescription calldata _sd) external payable override returns (uint256){
        ...
        if (success) {
            if(_sd.dstToken == ETH){
                return address(_sd.receiver).balance - toTokenBefore;
            }else{
    ...
    function swapOnUniswapV2Fork(
        ...
        //TODO 怎么知道兑换的目标token地址
        //        address toToken = pools[pools.length - 1];
